...
void Tas58xxComponent::loop() {
  ...
  switch (this->loop_setup_stage_) {
    ...
    case LR_VOLUME_SETUP:
      ...
      if (this->loop_setup_stage_ == LR_VOLUME_SETUP) this->loop_setup_stage_ = STAGE_AFTER_EQ;
      return;
    ...
    case EQ_BANDS_SETUP:
      ...
      if (this->refresh_band_ == NUMBER_EQ_BANDS) {     // refresh_band_ was initialised to 0
        ...
        this->loop_setup_stage_ = STAGE_AFTER_EQ;
    ...
    case DRC_SETUP:
      // Only reachable when USE_TAS58XX_DRC is defined - STAGE_AFTER_EQ routes
      // straight to SETUP_COMPLETE otherwise.
      //
      // Deliberately not spread across loop iterations the way the EQ bands
      // are. In three band mode the intermediate states are audible: after the
      // crossover is written but before the band mixer gains open up, band 1 is
      // lowpass-only and the output loses its highs. Writing the whole block in
      // one pass costs roughly 6ms of bus time once, which is the cheaper
      // trade.
      if (!this->apply_drc_all_()) {
        ESP_LOGW(TAG, "%s applying DRC settings", ERROR);
      }
  ...
  switch (this->error_code_) {
    ...
    case NONE:
      ...
      if (this->drc_is_three_band()) {
        ESP_LOGCONFIG(TAG,
                "  DRC: 3 band, crossover %.0fHz / %.0fHz\n"
...
bool Tas58xxComponent::set_mixer_mode(MixerMode mode) {
  ...
  if (!this->book_and_page_write_(TAS58XX_AUDIO_CTRL_BOOK, TAS58XX_MIXER_GAIN_PAGE, TAS58XX_MIXER_GAIN_SUBADDR,
    ...
    ESP_LOGW(TAG, "%s writing %s: %s", ERROR, MIXER_MODE, MIXER_MODE_TEXT[mode]);
    return false;
  }
...
bool Tas58xxComponent::set_eq_gain(Channels channel, uint8_t band_index, int8_t gain) {
  ...
  if ((eq_address == NULL) || (biquad == NULL)) {
    ESP_LOGE(TAG, "%s NULL discovered %s Channel %s:%d Gain: %ddB", ERROR, LR_CHANNEL_TEXT[channel], EQ_BAND, band, gain);
    return false;
  }
...
bool Tas58xxComponent::book_and_page_read_(uint8_t book, uint8_t page, uint8_t sub_addr, uint8_t* data, uint8_t number_bytes) {
  // Reads back a DSP coefficient. Unlike the write path this does not span a
  // page boundary - callers read one 4-byte coefficient at a time.
  bool ok = this->tas58xx_write_byte_(TAS58XX_PAGE_SET, TAS58XX_PAGE_ZERO);
  if (ok) ok = this->tas58xx_write_byte_(TAS58XX_BOOK_SET, book);
  if (ok) ok = this->tas58xx_write_byte_(TAS58XX_PAGE_SET, page);
  if (!ok) {
    ESP_LOGE(TAG, "%s setting book:0x%02X page:0x%02X to read", ERROR, book, page);
    return false;
  }

  const bool read_ok = this->tas58xx_read_bytes_(sub_addr, data, number_bytes);

  // Always return to book 0 page 0, even if the read failed, so a later
  // register access does not land on a stale page.
  ok = this->tas58xx_write_byte_(TAS58XX_PAGE_SET, TAS58XX_PAGE_ZERO);
  if (ok) ok = this->tas58xx_write_byte_(TAS58XX_BOOK_SET, TAS58XX_BOOK_ZERO);
  if (ok) ok = this->tas58xx_write_byte_(TAS58XX_PAGE_SET, TAS58XX_PAGE_ZERO);
  if (!ok) {
    ESP_LOGE(TAG, "%s re-setting book and page after read", ERROR);
    return false;
  }
  return read_ok;
}
