...
DrcEnableSwitch = tas58xx_ns.class_("DrcEnableSwitch", switch.Switch, cg.Component)
...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_ENABLE_DRC): switch.switch_schema(
            DrcEnableSwitch,
            device_class=DEVICE_CLASS_SWITCH,
...
async def to_code(config):
  ...
  if enable_drc_config := config.get(CONF_ENABLE_DRC):
    cg.add_define("USE_TAS58XX_DRC")
    s = await switch.new_switch(enable_drc_config)
    await cg.register_component(s, enable_drc_config)
    await cg.register_parented(s, tas58xx_component)
